...
futurefunk <- get_playlist_audio_features("", "37i9dQZF1DXbjGYBfEmjR5?si=4cdc10c06f8342df")
soft_lofi <-
  bind_rows(
...
Tempograms {.storyboard data-navmenu=Visualisation}
=========================================
...
### Archetypes = Future Funk

```{r}
futurefunk1 <- get_tidy_audio_analysis("5sd7kMz5p770fYhcD9w4rd?si=4703a27e8dc24c5b")
futurefunk1 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
Novel Functions {.storyboard data-navmenu=Visualisation}
=========================================

### Energy-based

```{r}
lofi1 <- get_tidy_audio_analysis("5yzk0dn1DmFcTkon5JrtTc?si=61052cc7cd0741dd") |>
  select(segments) |>
  unnest(segments)
chopin <- get_tidy_audio_analysis("40pgdwioxZSCUyCke0zzOp?si=c5c8c0e45dd54c76") |>
  select(segments) |>
  unnest(segments)
futurefunk1 <- get_tidy_audio_analysis("5sd7kMz5p770fYhcD9w4rd?si=4703a27e8dc24c5b") |>
  select(segments) |>
...
novel_functions <-
  bind_rows(
    futurefunk1 |> mutate(category = "Future Funk"),
    blackpink |> mutate(category = "Beast Mode"),
    chopin |> mutate(category = "Soft Classical"),
    lofi1 |> mutate(category = "Study Lofi")
  ) 
novel_functions |>
